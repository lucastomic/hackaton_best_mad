...
MakumbaAutoComplete.AutoComplete = function(id, location, type, field, fieldType, queryLang) {
	...
	if(fieldType == 'char') {
		...
		new Ajax.Autocompleter(id, 'autocomplete_choices_'+id, ''+location+'?type='+type+'&field='+field+'&fieldType='+fieldType+'&queryLang='+queryLang, { minChars: 2, paramName: 'value', autoSelect: true});
		
	} else if (fieldType == 'ptr') {
		
		new Ajax.Autocompleter(id + '_visible', 'autocomplete_choices_'+id, ''+location+'?type='+type+'&field='+field+'&fieldType='+fieldType+'&queryLang='+queryLang, { minChars: 2, paramName: 'value', autoSelect: true, afterUpdateElement: MakumbaAutoComplete.getSelectionId});
